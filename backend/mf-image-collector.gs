...
function mfImageCollectorApproveCurrentCategoryNormalizations() {
  ...
  var response = ui.alert(
    '現在カテゴリ正規化を一括承認',
    '現在カテゴリ正規化の未処理' + plan.eligible_rows.length + '件を「既存銘柄を更新」に設定します。\nMasterへの反映はまだ行いません。',
    ui.ButtonSet.YES_NO
  );
  ...
  var result = { ok: true, target_count: plan.target_count, updated: plan.eligible_rows.length, skipped: plan.skipped };
  ui.alert(
    '一括承認完了',
    '対象件数: ' + result.target_count + '\n更新件数: ' + result.updated + '\nskip件数: ' + result.skipped + '\n\nMasterは変更していません。',
    ui.ButtonSet.OK
  );
  return result;
}
...
function mfImageCollectorAuditVanillaTags() {
  ...
  SpreadsheetApp.getUi().alert(
    'ヴァニラタグ監査',
    '対象行数: ' + result.target_rows + '\nバニラ検出件数: ' + result.detected + '\nレビュー追加数: ' + result.created + '\n重複skip数: ' + result.duplicate_skips,
    SpreadsheetApp.getUi().ButtonSet.OK
  );
  return result;
}
...
function mfImageCollectorApplySelectedReviewRow() {
  ...
  if (salesSku) {
    ...
    lines.push('SKU追加先VersionKey: ' + (parent && parent.version_key ? parent.version_key : '（未特定）'));
    lines.push('SKU追加先Primary: ' + (parent && parent.reference ? parent.reference : '（未特定）'));
    lines.push('SKU追加先銘柄名: ' + (parent && parent.name ? parent.name : '（未特定）'));
  }
...
function mfImageCollectorReviewSalesSkuIdentity_(review) {
  ...
  var mode = mfImageCollectorReviewReferenceMode_(review);
  return {
    mode: mode,
    is_sales_sku: mode === 'sales_sku' && !!(displaySku || jsonSku),
    is_independent_hybrid_primary: mode === 'independent_primary' && !!(displaySku || jsonSku),
...
function mfImageCollectorReviewReferenceMode_(review) {
  var info = mfImageCollectorReviewOfficialInfo_(review);
  var resolution = info.reference_resolution && typeof info.reference_resolution === 'object' ? info.reference_resolution : {};
  var mode = String(resolution.mode || '').trim();
  if (['existing_primary', 'sales_sku', 'independent_primary', 'unresolved'].indexOf(mode) >= 0) return mode;

  var detectionType = String(review['検出種別'] || '').trim();
  var reference = String(info.primary_reference || review['Tリファレンス番号'] || '').trim().toUpperCase();
  var skuInfo = mfImageCollectorSalesSkuInfo_(reference);
  if (detectionType === 'sales_sku_detected' && skuInfo) {
    return mfImageCollectorLegacySalesSkuParentIsCurrent_(review, skuInfo) ? 'sales_sku' : 'unresolved';
  }
  if (detectionType === 'unregistered_reference' || detectionType === 'unregistered_reference_image') {
    if (/^T\d+$/.test(reference) || mfImageCollectorIsTfbfReference_(reference)) return 'independent_primary';
    var independentByType = String(info.primary_reference_type || '').trim() === 'independent_hybrid_primary';
    var independentByFlags = info.independent_primary === true && info.hybrid_reference === true && info.sku_only !== true;
    if (skuInfo && (independentByType || independentByFlags) && !String(info.t_reference || '').trim()) return 'independent_primary';
  }
  return 'unresolved';
}

function mfImageCollectorLegacySalesSkuParentIsCurrent_(review, skuInfo) {
  var info = mfImageCollectorReviewOfficialInfo_(review);
  var displayReference = String(review['Tリファレンス番号'] || '').trim().toUpperCase();
  var jsonReference = String(info.primary_reference || '').trim().toUpperCase();
  var displaySku = mfImageCollectorSalesSkuInfo_(displayReference);
  var jsonSku = mfImageCollectorSalesSkuInfo_(jsonReference);
  if (displaySku && jsonSku && displaySku.sku !== jsonSku.sku) return false;
  if (!skuInfo || (displaySku && displaySku.sku !== skuInfo.sku) || (jsonSku && jsonSku.sku !== skuInfo.sku)) return false;

  var resolution = info.reference_resolution && typeof info.reference_resolution === 'object' ? info.reference_resolution : {};
  var parentResolution = resolution.parent_resolution && typeof resolution.parent_resolution === 'object' ? resolution.parent_resolution : {};
  var parent = parentResolution.parent && typeof parentResolution.parent === 'object' ? parentResolution.parent : parentResolution;
  var savedVersionKeys = [
    review['対象VersionKey'], review['DB既存VersionKey'],
    parent.version_key, parent.target_version_key, info.sales_sku_target_version_key
  ].map(function(value) { return String(value || '').trim().toUpperCase(); }).filter(Boolean);
  savedVersionKeys = savedVersionKeys.filter(function(value, index) { return savedVersionKeys.indexOf(value) === index; });
  if (savedVersionKeys.length !== 1) return false;

  var savedTReference = String(review['DB既存T'] || '').trim().toUpperCase();
  if (!/^T\d+$/.test(savedTReference)) return false;
  var reviewNames = [review['公式名'], review['DB既存名'], info.official_name, info.current_official_name, info.name]
    .map(mfImageCollectorNormalizeNameForCompare_)
    .filter(Boolean);
  reviewNames = reviewNames.filter(function(value, index) { return reviewNames.indexOf(value) === index; });
  if (!reviewNames.length) return false;
  ...
  try {
    ...
    var values = sheet.getDataRange().getValues();
    var headers = values[0].map(function(value) { return String(value).trim(); });
    var candidates = mfImageCollectorSalesSkuParentRows_(values, headers).filter(function(candidate) {
      if (candidate.version_key !== savedVersionKeys[0] || candidate.reference !== savedTReference) return false;
...
function mfImageCollectorReviewConfirmationLabel_(review) {
  ...
  if (String(review['source_type'] || '').trim() === 'tag_spelling_normalization') return 'ヴァニラ表記の統一';
  var detectionType = String(review['検出種別'] || '').trim();
...
function mfImageCollectorReviewCandidateToRow_(candidate, detectionId) {
  var candidateTargetColumn = String(candidate.target_column || '').trim();
  var candidateSuggestedValue = String(candidate.suggested_value || '').trim();
  if (String(candidate.detection_type || '').trim() === 'structured_fact' && mfImageCollectorIsVanillaTagColumn_(candidateTargetColumn)) {
    candidateSuggestedValue = mfImageCollectorNormalizeVanillaTagValue_(candidateSuggestedValue);
  }
  var officialInfo = {
    ...
    reference_resolution: candidate.reference_resolution || {},
    ...
    sales_sku_target: candidate.sales_sku_target === true,
    ...
    target_provenance: ['human', 'resolver', 'legacy_unknown'].indexOf(String(candidate.target_provenance || '').trim()) >= 0
      ? String(candidate.target_provenance).trim()
      : 'legacy_unknown'
  };
  var row = {
    '検出ID': detectionId,
    ...
    '対象列': candidateTargetColumn,
    '現在値': String(candidate.current_value || '').trim(),
    '候補値': candidateSuggestedValue,
    '承認済み日本語説明': '',
...
function mfImageCollectorReviewIdentity_(candidate) {
  ...
  if (type === 'structured_fact') {
    var targetColumn = String(candidate.target_column || '').trim();
    var suggestedValue = String(candidate.suggested_value || '').trim();
    if (mfImageCollectorIsVanillaTagColumn_(targetColumn)) suggestedValue = mfImageCollectorNormalizeVanillaTagValue_(suggestedValue);
    return type + '|' + reference + '|' + String(candidate.existing_version_key || candidate.target_version_key || '').trim() + '|' + targetColumn + '|' + suggestedValue;
  }
...
function mfImageCollectorApplyApprovedReview_(review, decision, targetVersionKey, options) {
  ...
  if (decision === '新規銘柄として追加' || decision === '既存銘柄の新バージョンとして追加') {
    ...
    mfImageCollectorAssertAppendVersionKey_(
      values,
      headers,
      reference,
      versionKey,
...
function mfImageCollectorIsVanillaTagColumn_(targetColumn) {
  return targetColumn === '香味大分類' || targetColumn === '香味詳細タグ';
}

function mfImageCollectorNormalizeVanillaTagValue_(value) {
  return String(value || '').replace(/バニラ/g, 'ヴァニラ');
}

function mfImageCollectorEvaluateTagSpellingNormalization_(actualCurrentValue, candidateCurrentValue, suggestedValue) {
  var actual = String(actualCurrentValue || '').trim();
  var expected = String(candidateCurrentValue || '').trim();
  var incoming = mfImageCollectorNormalizeVanillaTagValue_(suggestedValue).trim();
  if (actual === incoming) return { already_applied: true, next_value: actual };
  if (actual !== expected) {
    throw new Error('Master value changed after tag spelling candidate was created: expected "' + expected + '" but found "' + actual + '".');
  }
  return { already_applied: false, next_value: incoming };
}
...
function mfImageCollectorReviewReferenceInfo_(review) {
  ...
  var referenceMode = mfImageCollectorReviewReferenceMode_(review);
  var independentHybridPrimary = referenceMode === 'independent_primary' && !!primarySku;
...
function mfImageCollectorSalesSkuRegistrations_(values, headers, sku) {
  var normalizedSku = String(sku || '').trim().toUpperCase();
  var versionCol = headers.indexOf('VersionKey');
  var primaryRefCol = headers.indexOf('Primary Reference');
  var referenceCols = mfImageCollectorSalesSkuReferenceColumns_(headers);
  var registrations = [];
  for (var i = 1; i < values.length; i += 1) {
    for (var j = 0; j < referenceCols.length; j += 1) {
      var tokens = mfImageCollectorDelimitedValues_(values[i][referenceCols[j]]).map(function(value) {
        return String(value).trim().toUpperCase();
      });
      if (tokens.indexOf(normalizedSku) < 0) continue;
      registrations.push({
        row_number: i + 1,
        version_key: versionCol >= 0 ? String(values[i][versionCol] || '').trim().toUpperCase() : '',
        primary_reference: primaryRefCol >= 0 ? String(values[i][primaryRefCol] || '').trim().toUpperCase() : ''
      });
      break;
    }
  }
...
function mfImageCollectorResolveSalesSkuParent_(review, targetVersionKey, skuInfo, values, headers) {
  ...
  if (explicitVersionKey) {
    ...
    return {
      row_number: explicitTarget.row_number,
      version_key: explicitTarget.version_key,
      reference: explicitTarget.primary_reference || explicitTarget.t_reference || '',
      name: explicitTarget.name || '',
      names: explicitTarget.name ? [mfImageCollectorNormalizeNameForCompare_(explicitTarget.name)] : [],
      url: ''
    };
  }
  ...
  if (existingVersionKey) {
    ...
    return {
      row_number: existingTarget.row_number,
      version_key: existingTarget.version_key,
      reference: existingTarget.primary_reference || existingTarget.t_reference || '',
      name: existingTarget.name || '',
      names: existingTarget.name ? [mfImageCollectorNormalizeNameForCompare_(existingTarget.name)] : [],
      url: ''
    };
  }

  // Automatic fallback remains conservative: infer only from current T-parent rows.
...
function mfImageCollectorAssertAppendVersionKey_(values, headers, reference, versionKey, requireNewPrimary) {
  var versionCol = headers.indexOf('VersionKey');
  var primaryRefCol = headers.indexOf('Primary Reference');
  if (versionCol < 0) throw new Error('VersionKey column is missing.');
  if (requireNewPrimary && primaryRefCol < 0) throw new Error('Primary Reference column is missing.');
  var ref = String(reference || '').trim().toUpperCase();
  var key = String(versionKey || '').trim().toUpperCase();
  mfImageCollectorAssertPrimaryReference_(ref);
  ...
  for (var i = 1; i < values.length; i += 1) {
    if (requireNewPrimary && String(values[i][primaryRefCol] || '').trim().toUpperCase() === ref) {
      throw new Error('Primary Reference already exists: ' + ref);
    }
...
function mfImageCollectorAuditVanillaTags_() {
  ...
  var refCol = headers.indexOf('Tリファレンス番号');
  var nameCol = headers.indexOf('現在の公式名');
  var tagColumns = ['香味大分類', '香味詳細タグ'].map(function(header) {
    return { header: header, index: headers.indexOf(header) };
  });
  if (versionCol < 0 || tagColumns.some(function(item) { return item.index < 0; })) {
    throw new Error('Master columns required for vanilla tag audit are missing.');
  }
  ...
  var reviewValues = reviewSheet.getLastRow() > 1
    ? reviewSheet.getRange(2, 1, reviewSheet.getLastRow() - 1, reviewSheet.getLastColumn()).getValues()
    : [];
  var result = { target_rows: 0, detected: 0, created: 0, duplicate_skips: 0 };
  for (var i = 1; i < values.length; i += 1) {
    var versionKey = String(values[i][versionCol] || '').trim();
    if (!versionKey) continue;
    result.target_rows += 1;
    for (var t = 0; t < tagColumns.length; t += 1) {
      var currentValue = String(values[i][tagColumns[t].index] || '').trim();
      if (currentValue.indexOf('バニラ') < 0) continue;
      result.detected += 1;
      var suggestedValue = mfImageCollectorNormalizeVanillaTagValue_(currentValue);
      if (mfImageCollectorHasOpenTagSpellingReview_(reviewValues, reviewHeaders, versionKey, tagColumns[t].header, suggestedValue)) {
        result.duplicate_skips += 1;
        continue;
      }
      var primaryReference = primaryRefCol >= 0 ? String(values[i][primaryRefCol] || '').trim() : '';
      var tReference = refCol >= 0 ? String(values[i][refCol] || '').trim() : '';
      var candidate = mfImageCollectorBuildVanillaTagReviewCandidate_({
        version_key: versionKey,
        reference: primaryReference || tReference || versionKey.replace(/-[BN]\d{2}$/i, ''),
        t_reference: tReference,
        name: nameCol >= 0 ? String(values[i][nameCol] || '').trim() : '',
        target_column: tagColumns[t].header,
...
function mfImageCollectorBuildVanillaTagReviewCandidate_(input) {
  return {
    detection_type: 'structured_fact',
    reference: String(input.reference || '').trim(),
    t_reference: String(input.t_reference || '').trim(),
    ...
    existing_name: String(input.name || '').trim(),
    target_version_key: String(input.version_key || '').trim(),
    target_column: String(input.target_column || '').trim(),
    current_value: String(input.current_value || '').trim(),
    suggested_value: mfImageCollectorNormalizeVanillaTagValue_(input.suggested_value),
    diff_summary: 'タグのヴァニラ表記を統一',
    evidence: 'タグ表記ルール: バニラ → ヴァニラ',
    evidence_text: 'タグ表記ルール: バニラ → ヴァニラ',
    source_type: 'tag_spelling_normalization',
    confidence: 'high',
    status: '要確認'
  };
}

function mfImageCollectorHasOpenTagSpellingReview_(values, headers, versionKey, targetColumn, suggestedValue) {
  var statusCol = headers.indexOf('ステータス');
  var typeCol = headers.indexOf('検出種別');
  ...
  var targetCol = headers.indexOf('対象列');
  var suggestedCol = headers.indexOf('候補値');
  if (statusCol < 0 || typeCol < 0 || sourceCol < 0 || targetCol < 0 || suggestedCol < 0) return false;
  for (var i = 0; i < values.length; i += 1) {
    if (MF_IMAGE_COLLECTOR_REVIEW_APPLY_STATUSES.indexOf(String(values[i][statusCol] || '').trim()) < 0) continue;
    if (String(values[i][typeCol] || '').trim() !== 'structured_fact') continue;
    if (String(values[i][sourceCol] || '').trim() !== 'tag_spelling_normalization') continue;
    var rowVersion = versionCol >= 0 ? String(values[i][versionCol] || '').trim() : '';
    if (!rowVersion && dbVersionCol >= 0) rowVersion = String(values[i][dbVersionCol] || '').trim();
    if (rowVersion !== String(versionKey || '').trim()) continue;
    if (String(values[i][targetCol] || '').trim() !== String(targetColumn || '').trim()) continue;
    if (String(values[i][suggestedCol] || '').trim() === String(suggestedValue || '').trim()) return true;
  }
  return false;
}
